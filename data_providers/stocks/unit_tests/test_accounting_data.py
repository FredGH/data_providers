...
class TestAccountingData(unittest.TestCase):
    ...
    def test_get_quarterly_balance_sheets_positive(self):
        # Prepare mock data
        mock_quarterly_balance_sheet = pd.DataFrame(
            {"Quarter": [1, 2], "Assets": [1000, 1200]}
        )
    ...
    def test_get_cashflow_positive(self):
        ...
        mock_cashflow = pd.DataFrame(
            {"Cash from Operations": [300], "Cash from Investing": [-100]}
        )
    ...
    def test_get_quarterly_cashflow_positive(self):
        # Prepare mock data
        mock_quarterly_cashflow = pd.DataFrame(
            {"Quarter": [1, 2], "Cash from Operations": [150, 200]}
        )
